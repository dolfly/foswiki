...
sub render {
	...
	my ($defsRef,$attrRef,$mandRef,$titlRef)  = _readDataFormsDefFromTopic($theWeb);
...
sub _renderUserLayout {
	...
	foreach my $name (keys %{$a}) {
		...
		#SMELL: Use Foswiki regex instead
		$title=~s/\W//g;
		$name=~s/\W//g;
		print STDERR "title: $title, name: $name";
...
sub handleRENDERFIELD {
	my ($cgi, $params, $topic, $web) = @_;
	local(%options, @unknownParams, @missingParams, @invalidParams);

	my $formDef = $params->{_DEFAULT};
	$params->{_DEFAULT} = undef; # for _initOptions()
	_initDefaults() unless $defaultsInitialized;
	#SMELL: join($params... because initOptions expects arg string, not hash 
	my $attributes = '';
	foreach my $key (keys %$params) {
		$attributes .= $key . '="' . $params->{$key} . '" ' if ($params->{$key});
	}
	_initOptions($attributes, $topic, $web) 
		|| return (($#unknownParams != -1)  ? _createUnknownParamsMessage() : 
		(($#missingParams != -1) ? _createMissingParamsMessage() : 
		_createInvalidParamsMessage()));

	print STDERR "formdef: $formDef\n";
	($mydefs,$_attr,$_mand,$_titl) = _readDataFormsDef($formDef);
	my ($mytd, $mytdadd) = _renderFormField($cgi, @$mydefs[0], $params->{form});
	return $mytd;
}
# =========================
...
# =========================
sub _readDataFormsDefFromTopic {
	my ($theWeb) = @_;
...
sub _readDataFormsDef {
	...
	foreach my $line (split(/[\r\n]+/, $data)) {
		...
		push @defs, { name=>$name, title=>$title,  type=>_get($cols[2],'text'), size=>_get($cols[3],80), value=>_get($value,""), values=>\@values, tooltip=>_get($cols[5],$cols[1]), attr=>_get($cols[6],"") };
		$attr{$name}=$defs[$#defs];
